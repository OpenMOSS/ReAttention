...
class InternLM2Attention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                ...
                print(f'dynamic ! with theta = {self.rope_theta} and scaling_factor = {scaling_factor}')
    ...
    def forward(
        ...
        #     q_dim_idx = torch.cat([torch.arange(self.num_key_value_groups * self.head_dim) + i * (self.num_key_value_groups + 2) * self.head_dim for i in range(self.num_key_value_heads)])
        #     k_dim_idx = torch.cat([torch.arange(self.head_dim) + self.num_key_value_groups * self.head_dim + i * (self.num_key_value_groups + 2) * self.head_dim for i in range(self.num_key_value_heads)])
        #     v_dim_idx = torch.cat([torch.arange(self.head_dim) + (self.num_key_value_groups+1) * self.head_dim + i * (self.num_key_value_groups + 2) * self.head_dim for i in range(self.num_key_value_heads)])

        #     q_proj_weight = self.wqkv.weight[q_dim_idx, :]
        #     k_proj_weight = self.wqkv.weight[k_dim_idx, :]
        #     v_proj_weight = self.wqkv.weight[v_dim_idx, :]

        #     past_key_value.init_compress(q_proj_weight, k_proj_weight, v_proj_weight, 
...
class InternLM2FlashAttention2(InternLM2Attention):
    ...
    def forward(
        ...
        # key_states = repeat_kv(key_states, self.num_key_value_groups)
        # value_states = repeat_kv(value_states, self.num_key_value_groups)  # added in 0714
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            elif len(past_key_values) == 0 or len(past_key_values[0]) == 0:
